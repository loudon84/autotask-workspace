    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="grid gap-4 pt-4 sm:grid-cols-2">
          ...
          {extraFieldsForCategory(portal.category).map((field) => (
            <Field
              key={field.key}
              label={field.label}
              value={portal.extra?.[field.key] || "-"}
            />
          ))}
          <Field
            label="创建人"
          ...
          <Field
            label="创建时间（北京时间）"
            value={formatBeijingDateTime(portal.createdAt)}
          />
          <Field
            label="更新时间（北京时间）"
            value={formatBeijingDateTime(portal.updatedAt)}
          />
        </CardContent>
      </Card>
      ...
      <WorkflowBindingManager portal={portal} />
    </div>
  );
}
