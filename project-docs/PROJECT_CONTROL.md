...
9. 当前测试部署使用 PostgreSQL 数据库 `nodeskclaw_task`、Engine 专属 schema `rpa_engine` 及九张 Engine 专属表。跨服务引用继续以外部字符串保存，不对 Task 专属表建立外键。
    10. 可以准备数据库设计和 DDL；未另行授权前不创建库、不执行其它 DDL。**v5.1 迁移 `f1a9c3e74b20` 已于 2026-08-24 经用户授权执行。v5.2 迁移 `g3b8e2a91c40`（`scheduler_jobs`）同日已执行。v5.1 `is_task_admin` 迁移 `a7e4b2c81d09` 已于 2026-08-25 经用户授权执行。v5.4 `integration_call_logs` 迁移 `b8c9d0e12f51` 已于 2026-08-27 经用户授权执行。门户创建人/归属人姓名字段 `c1d8e4f90a62` 已于 2026-08-28 执行。v5.5 `portal_accounts.category` 迁移 `d2e9f1a70b83` 已于 2026-09-03 经用户授权在测库执行。调度中心 `timers` 迁移 `c3a8f1d92e47` 已于 2026-09-04 经用户授权在测库执行。执行记录 `timer_runs` 迁移 `d4b2f7a91e05` 同日已执行。BOE 地区表 `b2d4f6a81935` 测库已有。门户 JSONB `extra` 迁移 `e7c2b9d04a18` 已于 2026-09-08 经用户授权在测库执行。流程实例开放唯一索引 `f8c2e91b4a70` 已于 2026-09-10 经用户授权在测库执行（当前测库 head）。正式库 `rpa_autotask` 已于 2026-09-10 经用户授权迁到 `d4b2f7a91e05`（category / 分类文档 / timers / timer_runs）。正式库未迁 extra / 地区表 / 本索引。**
...
| 调度中心 | **清场已做（2026-09-09）** | 进程只留 `TimerScheduler`。打开开关仍会跑，不改 target/入口/已有开关。文档 `prd/AutoTask 调度中心-清场.md`（原文不动）。**需重启 4520**。正式库 `timers` 已于 2026-09-10 迁到 `d4b2f7a91e05`；catalog 行要等正式 Task 再启动才插入，默认停用。 |
...
| 京东方发票箱单（BOE） | **一期已落地；09-10 体验优化已改** | 流水号可点进详情（空为 —）。重试只在失败时出现。子任务改抽屉。净重单位默认千克。WMS 地区未维护则停本阶段。删草稿 1.0.1。排重索引测库已迁。**需重启 4520**。设计 `prd/boe/AutoTask-BOE v1.0 设计-发票箱单SOP.md` |
| 门户和流程实例优化（v5.5） | **测库已到 `f8c2e91b4a70`；正式库 `d4b2f7a91e05`** | 测库含 extra 与排重索引。正式 `rpa_autotask` 已迁 category / 分类文档 / timers。门户「天地伟业-芯云」仍在，`category=TIANDI`。正式未迁 extra / 地区表 / 排重索引，**禁止对正式 `upgrade head` 到测库 head**。见 PRD §5.4 / §8。 |
...
25. **v5.5 门户分类 + 分类文档**：测试库已过该点。正式库已于 2026-09-10 迁到 `d4b2f7a91e05`（含 `category` 与 `category_documents`），门户未删、回填 TIANDI。**需用 `.env.product` 再启一次正式 4520** 才会插入 timers catalog。正式不要再 `upgrade head` 到测库 head（后面还有 BOE extra / 地区表 / 排重索引）。见 PRD §8。
26. **调度中心清场（2026-09-09）**：只留独立定时器循环。见 `prd/AutoTask 调度中心-清场.md`。**需重启 4520**。正式 `timers` 表已迁；catalog 行等正式 Task 再启动。
...
- **正式库已迁到 d4b2f7a91e05（用户授权）**：`rpa_autotask` `c1d8e4f90a62` → `d4b2f7a91e05`。连续四步：`category` 回填 TIANDI、`category_documents`、`timers`、`timer_runs`。未覆盖测库 `.env`。门户「天地伟业-芯云」仍 ENABLED，未删。正式未迁 extra / 地区表 / 排重索引。**下一步：用 `.env.product` 再启一次正式 Task**（先前那次启动早于迁表，timers catalog 是空的）。

- **生成对账单金额不一致改为可确认继续**：原先 SDMS 金额与勾选汇总不一致会 409 强制拦截（「请去 SDMS 修改对账单后重新发起」）。现仍提示两边金额；用户点「仍要生成」后带 `confirmAmountMismatch` 继续落草稿，summary 记 `amount_mismatch_confirmed`。未找到 SDMS 对账单仍拦截。Client 需重装安装包。

- **发票箱单 09-10 体验**：列表流水号可点进详情（没有则 —）。重试只在有错误或任务 FAILED 时出现，执行中不显示。流程详情点子任务改右侧抽屉，不离开当前页。净重单位取不到固定千克。WMS 地区未维护则停在读 WMS，维护后重试再进补全。**需重启 4520**。
...
- **BOE v2.1 操作流程已定**：扫计划成功即建实例（仅头）；拉 WMS 独立任务，失败不重扫。第一次 RPA 补全行（PO+料号搜不到停本节点）。保存 SRM 草稿独立节点，失败把当前页回写 Client 再重试保存。保存成功冻结 `reviewBaseline`；提交 RPA 是变更单（diff 后改草稿）。设计已按 v2.1 改完。未写代码、未迁库。 **已被同日 v2.2 取代。**

- **BOE v2.0 操作流程已定**：每天 7 点扫交货计划（当天 + WMS 订单完成）落 Client 草稿（交货计划头 + WMS 行/体积/箱单发票）；一次 RPA 录 SRM **保存草稿**（PO+料号查凭证、回写行字段）；客服校验并手传双签 PO；再一次 RPA 打开草稿 **提交正式**。需求步骤 2+3 不拆成两次 Run。地区编号 Client 配置表映射 SRM 名称。设计已重写。未写代码、未迁库。 **已被同日 v2.1 取代。**

- **BOE 登录态与天地伟业同一套**：Engine Cookie 缓存仍可用于少走登录页。**邮箱验证码记在 SRM 后台（账号+当天），不是 Cookie**——A 电脑输过，B 电脑当天同账号也不要码。不能复用的是未保存新建表单。Client 只存指令，提交 Run 对页校验。设计 §1.1、§6.4。

- **BOE 不拆多个 Run**：补充「当天同账户后续免验证码」和「单据页只能从导航点入」。仍维持一张单一提交 Run。ticket / 新建页 URL 禁止写进 Flow。
...
- **流程模板/Binding 显示名加上「天地伟业-」**：8 条模板已改名（扫待签章、建 SDMS、回签轮询、下合同、收货查询、生成对账单、上传发票、提交审核）。Binding 无独立名称字段，门户 Binding 卡片和任务列表读模板名，刷新 Client 即见。code 未改。仍是阶段 A 正式演练闸。

- **新环境阶段 A 已初始化（正式演练闸）**：MinIO 新桶 `rpa-flow-packages-production`。正式包已重新发布到空 Registry。门户「天地伟业-芯云」（正式 SRM）已绑：扫单 1.1.3（样例 PO + treatAsPending，调度关）、建单 1.2.20、回签 1.1.4（调度关）、下合同 1.3.3、收货 1.1.3、生成 1.1.0 dryRun、扫描发票 1.1.2、提交 1.1.5 dryRun。未绑填交期/签章。

- **新库 rpa_autotask 已迁表**：Task head `b8c9d0e12f51`（空库走 create_all + stamp，26 张业务表）；Engine schema `rpa_engine` head `20260713_0001`（10 张表）。Engine 配置不再写死库名必须是 `nodeskclaw_task`。下一步：启新 4520/4610、建门户，再发 Flow 绑 Binding。

- **上线步骤改成两段**：新应用+新空库。测试 `.env` 可当模板拷走，只改库和本机接线，不拷测试业务数据。阶段 A 做成和「天地伟业-芯云-正式演练」一样（测 SDMS/ERP、样例扫单、生成/提交 dryRun、调度关）。确认没问题后再阶段 B 收样例单、关 dryRun、`.env` 切生产、开调度。文档：`prd/AutoTask 正式上线操作清单.md`。
