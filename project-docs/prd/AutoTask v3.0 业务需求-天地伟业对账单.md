...
> **文档状态（2026-08-21）**：**定稿 + 增量修订**。三轮 22 项确认仍有效；待生成草稿见 #23–#25。**#26–#28 已修订（2026-08-21）**：扫描与提交拆成两次 RPA；客服在 Client 核对第一次回写的发票号/总额后再点提交；提交 RPA 必须再扫一次且结果与页面一致，否则失败不提交。原「禁止单独扫描」因客服无法核验而废止。
>
...
3. SRM **没有**独立的「已上传未提交」落态。只扫描不提交时刷新页面，门户附件会消失。因此 **提交审核必须再扫一次**；第一次扫描只为把发票号/总额回写到 Client 供核验。
...
1. 客服选择发票后点「扫描发票」（单独 RPA）。成功后本地回写发票号、发票总额，对账状态仍为未对账。
2. 客服在 Client 详情核对发票号、发票总额（及与对账总额是否合理）。不对则换文件再扫。
3. 点「提交审核」表示页面上的发票信息已审核通过。前置：已有发票号/总额，且提交文件与上次扫描相同。
4. **提交 RPA** 在同一浏览器会话内：定位对账单 → **再扫一次同一批文件** → 回写必须与页面已核对的发票号/总额一致 → 一致才点提交审核。不一致则失败，不点提交。
5. 成功后本地对账状态 = 已对账，发票状态 = 审批中。

**失败处理：**

- 不做自动重试，只记录操作失败原因并展示给用户。
- 扫描失败或二次扫描不一致：本地保持未对账；可重新扫描或重新提交。
- 门户刷新后第一次扫描的附件会消失，这是预期。
...
| 26 | 上传发票能否单独跑 RPA | **是（2026-08-21 修订）**。第一次扫描单独跑，只为 Client 展示发票号/总额。门户刷新附件仍会消失，所以提交时必须再扫一次 |
| 27 | Client 何时触发 RPA | 选文件后点「扫描发票」发起第一次；核对通过后点「提交审核」发起第二次 |
| 28 | 提交前是否必须已有发票号/金额 | **是（2026-08-21 修订）**。页面上的发票号/总额是客服已核对通过的基准；提交 RPA 二次扫描必须与之一致，否则失败且不点门户提交 |

### 6.4 2026-08-21 修订（核验发票）

原 #26–#28 把扫描和提交焊在一次 RPA 里，客服在提交前看不到回写结果，无法核验。现改为：

1. 第一次扫描 → 页面展示发票号、发票总额 → 人核对  
2. 点提交审核 = 确认页面结果通过  
3. 第二次扫描必须等于页面结果，否则视为数据错乱，禁止提交  
