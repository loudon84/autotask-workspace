...
2. 待签章详情页使用订单专属下载文件时，仍必须通过相同 XLSX 安全检查和字段校验；测试阶段暂不校验 XLSX 订单编号与任务 `po_no` 是否一致。
...
- 任务输入 `po_no` 仍用于门户搜索和详情页导航，Flow 仍需确认打开的是该任务指定的门户订单。
- 下载的 XLSX 仍必须包含合法且非空的“订单编号”列，但暂不与任务 `po_no` 比较，也不再返回 `ORDER_ATTACHMENT_PO_MISMATCH`。
- ERP 行字段 `custPoNumber` 完全保留 XLSX 中每一行的订单编号；若附件与任务订单不一致，ERP 将收到附件中的值。
- 成功输出顶层 `poNo` 仍表示任务输入，逐行业务明细则表示 XLSX 内容。测试人员必须同时核对这两类值，不能把本例外视为生产数据防错能力。
- 这是测试阶段的临时放宽。恢复生产门禁时必须发布新的不可变 Flow 版本，不能覆盖 `1.2.2`。
