...
SAVE_SUCCESS_TEXT = '已按明细行保存预计交货日期'
...
def validate_input(raw_input):
    value = raw_input if isinstance(raw_input, dict) else {}
...
def _dates_match(lines):
    return all(
        line['currentExpectedDate'] == line['expectedDeliveryDate']
        for line in lines
    )
...
class SupplierPortalDeliveryDateAdapter:
    ...
    async def login(self):
        step_id = 'srm.login'
        credentials = (
            self.ctx.credentials if isinstance(self.ctx.credentials, dict) else {}
        )
    ...
    async def open_order_detail(self, po_no):
        ...
        try:
            await self.page.goto(
                f'{portal_root}/#/supplier/pend-orders/{po_no}',
                wait_until='domcontentloaded',
            )
            ...
            raise RpaRetryableError(
                ...
                'Pending-signature order detail could not be verified',
            ) from exc
        await self.ctx.events.emit(
            'STEP_SUCCEEDED',
            message='Pending-signature order detail opened',
    ...
    async def ensure_editable(self, lines):
        try:
            save_button = self.page.locator(self.selector('save_all'))
            sign_button = self.page.locator(self.selector('sign'))
            if not await save_button.is_visible() or not await save_button.is_enabled():
                raise RpaBusinessError(
                    'ORDER_NOT_EDITABLE',
                    'Customer purchase order is not editable',
                )
    ...
    async def ensure_sign_not_executable(self):
        try:
            sign_button = self.page.locator(self.selector('sign'))
            await sign_button.wait_for(state='visible', timeout=5000)
    ...
    async def save_and_verify(self, po_no, lines):
        try:
            await self.page.locator(self.selector('save_all')).click(timeout=10000)
            success_message = await self._wait_for_action_result(
                success_selector='save_success',
                error_selector='save_error',
                success_text=SAVE_SUCCESS_TEXT,
                rejected_code='ORDER_DATE_SAVE_REJECTED',
                unknown_code='ORDER_DATE_SAVE_OUTCOME_UNKNOWN',
            )
        except asyncio.CancelledError:
            raise RpaHumanRequiredError(
                'ORDER_DATE_SAVE_OUTCOME_UNKNOWN',
                'Expected delivery date save requires manual verification',
            ) from None
        ...
        try:
            ...
            if not _dates_match(persisted):
                raise RpaHumanRequiredError(
                    ...
                    'Saved expected delivery dates require manual verification',
                    details={
                        'lineNumbers': [
                            line['lineNo']
                            for line in persisted
                            if line['currentExpectedDate']
                            != line['expectedDeliveryDate']
                        ]
                    },
                )
            return success_message, persisted
        except asyncio.CancelledError:
            raise RpaHumanRequiredError(
                'ORDER_DATE_PERSISTENCE_UNCONFIRMED',
                'Saved expected delivery dates require manual verification',
...
async def run(ctx):
    ...
    await ctx.events.emit(
        'STEP_SUCCEEDED',
        message='Expected delivery dates filled and stable before save',
        payload={
            'stepId': 'srm.fill_delivery_dates',
            'poNo': po_no,
            'lineCount': len(lines),
    ...
    await ctx.events.emit(
        'STEP_STARTED',
        message='Saving expected delivery dates',
        payload={
            'stepId': 'srm.save_delivery_dates',
            'stepType': 'srm.save_delivery_dates',
            'poNo': po_no,
            'lineCount': len(lines),
        },
    )
    try:
        _save_message, lines = await adapter.save_and_verify(po_no, lines)
        try:
            await adapter.capture_stable_screenshot(
                'supplier-portal-delivery-dates-saved',
                'srm.save_delivery_dates',
                lines,
            )
        except Exception as exc:
            raise RpaHumanRequiredError(
                'ORDER_DATE_PERSISTENCE_UNCONFIRMED',
                'Saved page could not be stabilized for verification evidence',
            ) from exc
    except (RpaBusinessError, RpaHumanRequiredError) as error:
        await _post_action_failure(
            ctx,
            adapter,
            step_id='srm.save_delivery_dates',
            screenshot_name='supplier-portal-delivery-dates-save-failed',
            error=error,
            po_no=po_no,
        )
        raise
    await _safe_emit(
        ctx,
        'STEP_SUCCEEDED',
        message='Expected delivery dates saved and verified',
        payload={
            'stepId': 'srm.save_delivery_dates',
            'poNo': po_no,
            'lineCount': len(lines),
