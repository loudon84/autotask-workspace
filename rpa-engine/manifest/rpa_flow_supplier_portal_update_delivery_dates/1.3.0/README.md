...
5. 签章结果必须明确同时包含“签章成功”和“已回签”。
6. 记录动作已接受截图并直接返回成功，以触发 Task 后继任务。

> 演示兼容说明：`1.3.0` 专用于当前演示门户。门户成功提示出现后，不再刷新页面验证回复状态和日期持久化，因此成功输出会明确标记 `verificationMode=ACTION_CONFIRMATION_ONLY`、`persistenceVerified=false`。需要严格持久化确认时继续使用 `1.2.0`。

成功提示缺少“已回签”、点击超时、断连、取消或结果不明确时，Flow 进入 `WAITING_HUMAN / ORDER_SIGN_OUTCOME_UNKNOWN`，不会自动再次点击。
...
- `supplier-portal-delivery-dates-sign-accepted-demo`：门户明确返回签章成功及已回签提示后立即记录；该截图不证明刷新后的持久化结果。
...
```json
{
  ...
  "signed": true,
  "replyStatus": "已回签",
  "verificationMode": "ACTION_CONFIRMATION_ONLY",
  "persistenceVerified": false,
  "lines": []
}
```

`schemaVersion` 用于 Task 服务在任务 2 成功后映射并自动排队任务 3。为保持下游契约兼容，`saved` 仍为 `true`；但 `1.3.0` 必须同时读取 `persistenceVerified=false`，不能把本版本的成功结果解释为刷新后持久化已验证。
