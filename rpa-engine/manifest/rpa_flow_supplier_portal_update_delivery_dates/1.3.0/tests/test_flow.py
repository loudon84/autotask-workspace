...
class LoginCredentialMappingTests(unittest.IsolatedAsyncioTestCase):
    async def test_accepts_engine_read_only_credentials_mapping(self):
        class Locator:
            ...
            async def get_attribute(self, name):
                self.page.attributes.append((self.selector, name))
                return "/captcha/code03.png"
            ...
            async def is_visible(self):
                return self.selector == "[login-success]"
        ...
        class Page:
            ...
            async def wait_for_timeout(self, timeout):  # noqa: ASYNC109
                raise AssertionError(f"Unexpected login wait: {timeout}")
        ...
        self.assertEqual(
            page.gotos,
            [("http://portal.test/", "domcontentloaded")],
        )
        self.assertIn(("[username]", "tester"), page.fills)
        self.assertIn(("[password]", "test-password"), page.fills)
        self.assertIn(("[captcha]", "sez0"), page.fills)
        self.assertEqual(page.checks, ["[agreement]"])
...
class RunOrchestrationTests(unittest.IsolatedAsyncioTestCase):
    async def test_normal_multi_line_direct_sign_success_in_demo_mode(self):
        calls = []
        ...
        class FakeAdapter:
            ...
            async def save_and_verify(self, *_args):
                raise AssertionError("The direct-sign flow must not save separately")
        ...
        self.assertEqual(
            ...
            "ORDER_DELIVERY_CONFIRMATION_OUTPUT_V1",
        )
        self.assertTrue(result["saved"])
        self.assertTrue(result["signed"])
        self.assertEqual(result["replyStatus"], "已回签")
        self.assertEqual(result["verificationMode"], "ACTION_CONFIRMATION_ONLY")
        self.assertFalse(result["persistenceVerified"])
...
class PackageContractTests(unittest.TestCase):
    def test_manifest_and_selectors_use_line_array_and_sign_action(self):
        ...
        self.assertNotIn("save_all", selectors)
        self.assertNotIn("save_success", selectors)
        self.assertNotIn("save_error", selectors)

    def test_flow_does_not_reference_any_save_control(self):
        ...
        self.assertNotIn("save_and_verify", source)
        self.assertNotIn("srm.save_delivery_dates", source)
        self.assertNotIn("pend-order-detail-save-btn", source)
...
class FakeActionPage:
    def __init__(self, selectors, *, sign_error=None):
...
class ActionBoundaryTests(unittest.IsolatedAsyncioTestCase):
    def make_adapter(self, *, sign_error=None):
        selectors = {
            "sign": "[sign]",
            "detail_po_number": "[detail-{po_no}]",
            "reply_status": "[reply-status]",
        }
        page = FakeActionPage(selectors, sign_error=sign_error)
        ctx = SimpleNamespace(page=page, selectors=selectors)
        adapter = flow_module.SupplierPortalDeliveryDateAdapter(ctx)
        return adapter, page

    async def test_sign_clicks_once_and_accepts_explicit_demo_confirmation(self):
    ...
    async def test_sign_confirmation_returns_all_eight_requested_lines(self):
        adapter, page = self.make_adapter()
        expected_lines = [
            {
                "lineNo": str(line_no),
                "materialNo": f"MATERIAL-{line_no}",
                "expectedDeliveryDate": "2026-07-30",
            }
            for line_no in range(10, 81, 10)
        ]
        async def result(**_kwargs):
            return "签章成功，回复状态已更新为已回签"
        ...
        _message, status, lines = await adapter.sign_and_verify(
            "POJS2607130002",
            expected_lines,
        )
    ...
    async def test_sign_success_without_signed_status_requires_human(self):
        adapter, page = self.make_adapter()
