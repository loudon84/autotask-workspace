...
class SupplierPortalAttachmentAdapter:
    ...
    async def verify_signed(self, po_no):
        step_id = "srm.verify_signed"
        try:
            status_locator = self.page.locator(self.selector("reply_status"))
            await status_locator.wait_for(state="visible", timeout=10000)
            status = _clean(await status_locator.inner_text())
        except Exception as exc:
            raise RpaRetryableError(
                "ORDER_REPLY_STATUS_UNAVAILABLE",
                "Customer purchase order reply status could not be read",
            ) from exc
        if status != SIGNED_REPLY_STATUS:
            raise RpaBusinessError(
                "ORDER_NOT_SIGNED",
                "Customer purchase order is not signed",
                details={"replyStatus": status, "poNo": po_no},
            )
        await self.ctx.events.emit(
            "STEP_SUCCEEDED",
            message="Customer purchase order is signed",
            payload={"stepId": step_id, "poNo": po_no, "replyStatus": status},
        )
    ...
    async def download_order(self):
        ...
        await self.ctx.events.emit(
            "STEP_STARTED",
            message="Downloading signed customer purchase order attachment",
            payload={"stepId": step_id, "stepType": step_id},
        )
        try:
        ...
        await self.ctx.events.emit(
            "STEP_SUCCEEDED",
            message="Signed customer purchase order attachment downloaded",
            payload={
                "stepId": step_id,
...
async def run(ctx):
    ...
    await ctx.log.info(
        "Starting signed order attachment upload Flow",
        {"poNo": po_no},
    )
    ...
    await adapter.verify_signed(po_no)
    ...
    async with AttachmentSystemClient() as client:
        ...
        await _safe_emit(
            ctx,
            "STEP_STARTED",
            message="Uploading signed order attachment",
            payload={
    ...
    await _safe_emit(
        ctx,
        "STEP_SUCCEEDED",
        message="Signed order attachment uploaded and verified",
        payload={
            "stepId": "attachment.upload",
            "poNo": po_no,
