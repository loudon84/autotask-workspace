...
class InputAndContractTests(unittest.TestCase):
    ...
    def test_manifest_and_selectors_define_frozen_contract(self):
        ...
        self.assertNotIn("verify_signed", source)
        self.assertNotIn("ORDER_NOT_SIGNED", source)
...
class RunOrchestrationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reply_status_check_is_skipped_before_download_and_upload(self):
        class StatusProbeAdapter(FakeAdapter):
            async def verify_signed(self, _po_no):
                raise AssertionError("Task 3 must not read the reply status")
        ...
        self.assertTrue(result["uploaded"])
        self.assertIn(("portal", "download"), ACTIVE_TIMELINE)
        self.assertEqual(
            len(FakeAttachmentClient.instances[0].upload_calls),
            1,
        )
