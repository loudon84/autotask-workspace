"""Build + publish official-portal statement generate 1.1.1 to production Engine.

Writes _publish_1.1.1.product.json. Does not overwrite the local 4610 receipt.
"""
    ...
    with httpx.Client(timeout=60.0, trust_env=False) as client:
        ...
        with ZIP.open("rb") as fh:
            resp = client.post(
                ...
                data={
                    "scope": "GLOBAL",
                    "description": f"{FLOW_ID} {VERSION} official select-all count check",
                },
            )
        ...
        pub = client.post(
            ...
            json={"reason": f"publish {FLOW_ID} {VERSION} official generate select-all"},
        )
