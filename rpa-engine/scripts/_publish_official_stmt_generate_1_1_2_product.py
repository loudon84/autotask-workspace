"""Build + publish official-portal statement generate 1.1.2 to production Engine.

Writes _publish_1.1.2.product.json. Does not overwrite the local 4610 receipt.
"""
...
ENGINE = "http://192.168.102.247:4610"
...
RECEIPT = FLOWS / FLOW_ID / f"_publish_{VERSION}.product.json"
    ...
    with httpx.Client(timeout=60.0, trust_env=False) as client:
        ...
        with ZIP.open("rb") as fh:
            resp = client.post(
                ...
                data={
                    "scope": "GLOBAL",
                    "description": f"{FLOW_ID} {VERSION} header checkbox requires selected count",
                },
            )
        ...
        pub = client.post(
            ...
            json={"reason": f"publish {FLOW_ID} {VERSION} header checkbox selected-count guard"},
        )
