"""Build + publish official-portal statement generate 1.1.3 to local Engine.

Writes _publish_1.1.3.json. Does not overwrite the production receipt.
"""
...
RECEIPT = FLOWS / FLOW_ID / f"_publish_{VERSION}.json"
...
def main() -> None:
    ...
    with httpx.Client(timeout=60.0, trust_env=False) as client:
        live = client.get(f"{ENGINE}/health/live")
        print("engine", ENGINE)
        print("live", live.status_code, live.text[:200])
        live.raise_for_status()
        ready = client.get(f"{ENGINE}/health/ready")
        print("ready", ready.status_code, ready.text[:240])
        # Local 4520 is often down; registry still accepts packages when live.
        with ZIP.open("rb") as fh:
            resp = client.post(
                ...
                data={
                    "scope": "GLOBAL",
                    "description": (
                        f"{FLOW_ID} {VERSION} page size 100 then header checkbox"
                    ),
                },
            )
        ...
        pub = client.post(
            ...
            json={"reason": f"publish {FLOW_ID} {VERSION} page size 100 header checkbox"},
        )
        ...
        out = {
            ...
            "workflowCode": "srm_stmt_generate",
            "engine": ENGINE,
        }
        RECEIPT.write_text(
