"""Build + publish official-portal statement generate 1.1.3 to production Engine.

Writes _publish_1.1.3.product.json. Does not overwrite the local 4610 receipt.
"""
    ...
    with httpx.Client(timeout=60.0, trust_env=False) as client:
        ...
        with ZIP.open("rb") as fh:
            resp = client.post(
                ...
                data={
                    "scope": "GLOBAL",
                    "description": f"{FLOW_ID} {VERSION} page size 100 then header select-all",
                },
            )
        ...
        pub = client.post(
            ...
            json={"reason": f"publish {FLOW_ID} {VERSION} page-size-100 header select-all"},
        )
