"""Build + publish official-portal statement submit review 1.1.5.

dryRun still does not click 提交审核. Scan uploads must not be blocked.
Do not bind to demo.
"""
...
def main() -> None:
    ...
    with httpx.Client(timeout=60.0, trust_env=False) as client:
        with ZIP.open("rb") as fh:
            resp = client.post(
                ...
                data={
                    "scope": "GLOBAL",
                    "description": (
                        f"{FLOW_ID} {VERSION} official submit; "
                        "install write guard after invoice scan"
                    ),
                },
            )
        ...
        pub = client.post(
            ...
            json={"reason": f"publish {FLOW_ID} {VERSION} scan then write-guard"},
        )
