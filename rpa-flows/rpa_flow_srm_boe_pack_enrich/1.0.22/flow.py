...
async def _wait_popup_data(page, rows, *needles: str, timeout_ms: int = 15000) -> int:
    ...
    while time.monotonic() < deadline:
        matched = 0
        for idx in range(await _row_count(rows)):
        ...
        enriched.append(
            {
                ...
                "orderQty": line.get("orderQty") or parsed["orderQty"],
                "orderUnit": line.get("orderUnit") or parsed["orderUnit"],
                "netWeightUnit": line.get("netWeightUnit") or parsed["netWeightUnit"],
            }
        )
    return {
