...
def _live_page(page):
    """保存后当前窗口会关，改用浏览器里还活着的最后一页（通常是列表）。"""
    try:
...
async def _read_draft_no_from_list(page, sel, invoice_no: str) -> str:
    ...
    await _fill(page, sel("list_search"), invoice_no)
    await page.locator(sel("list_search_button")).first.click()
    await page.wait_for_timeout(1200)
    rows = page.locator(sel("list_row"))
    count = await _count(rows)
    ...
    draft_no = draft_no_from_list_row(blob, invoice_no)
    if not draft_no:
        raise RpaBusinessError(
            "BOE_DRAFT_NO_MISSING",
            f"列表首行不是发票号 {invoice_no} 的草稿：{blob[:120]}",
        )
    return draft_no
...
async def run(ctx):
    ...
    try:
        ...
        draft_no = await _read_draft_no_from_list(page, sel, packing["invoiceNo"])
        return {
