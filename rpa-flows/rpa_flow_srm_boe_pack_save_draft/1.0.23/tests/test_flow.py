...
def test_list_search_uses_supplier_invoice_placeholder() -> None:
    import json

    selectors = json.loads((FLOW_DIR / "selectors.json").read_text(encoding="utf-8"))
    assert "placeholder='供应商发票号'" in selectors["list_search"]
    assert "placeholder*='发票'" not in selectors["list_search"]
    assert "展开" in selectors["list_expand"]
    assert "placeholder='状态'" in selectors["list_status"]
    assert "button.el-button--text" in selectors["list_draft_no"]


def test_list_draft_button_regex_matches_yingdao_example() -> None:
    assert flow_module.LIST_DRAFT_NO_RE.fullmatch("I260907336")
    assert flow_module.LIST_DRAFT_NO_RE.fullmatch("td rowspan") is None


def test_is_pack_save_url() -> None:
    assert flow_module.is_pack_save_url(
        "https://bsrm.boe.com/apis/srm-invoice-packing-list/invoicepackinglist/create"
    )
    assert flow_module.is_pack_save_url(
        "https://bsrm.boe.com/apis/srm-invoice-packing-list/invoicepackinglist/update"
    )
    assert not flow_module.is_pack_save_url(
        "https://bsrm.boe.com/apis/srm-invoice-packing-list/invoicepackinglist/list?current=1"
    )
