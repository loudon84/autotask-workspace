...
async def run(ctx):
    ...
    # 演示开发阶段安全闸：禁止真实提交 SRM（最多保存草稿）。
    # 正式上线时由调用方显式传 allowRealSubmit=true 才放行；否则直接失败，
    # 防止误点 Client「提交 SRM 单据」或误配 Binding 造成真实提交。
    if payload.get("allowRealSubmit") is not True:
        raise RpaBusinessError(
            "BOE_SUBMIT_BLOCKED_DEMO",
            "演示开发阶段禁止真实提交 SRM（最多保存草稿）；"
            "正式上线需调用方显式传 allowRealSubmit=true",
        )
