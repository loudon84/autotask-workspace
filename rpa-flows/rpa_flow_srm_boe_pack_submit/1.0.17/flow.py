...
async def _find_item_row(page, sel, po_num: str, item_num: str):
    ...
    for idx in range(await _count(rows)):
        row = rows.nth(idx)
        try:
            blob = str(await row.inner_text() or "")
        except Exception:
            continue
        if po_num and po_num in blob:
            return row
    return None


async def _apply_line_field_updates(page, sel, line: dict, fields: dict) -> None:
    row = await _find_item_row(
...
async def _apply_diffs(page, sel, packing: dict) -> None:
    ...
    for item in packing["lineDiffs"]:
        if item["action"] == "update":
            await _apply_line_field_updates(page, sel, item["line"], item.get("fields") or {})
            continue
        if item["action"] != "add":
            continue
        line = item["line"]
