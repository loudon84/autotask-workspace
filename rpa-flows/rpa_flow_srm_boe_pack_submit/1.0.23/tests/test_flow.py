...
def test_list_search_uses_draft_no_placeholder() -> None:
    selectors = __import__("json").loads((FLOW_DIR / "selectors.json").read_text(encoding="utf-8"))
    assert selectors["list_search"] == ".invoice-list input[placeholder='发票箱单流水号']"
    assert "展开" in selectors["list_expand"]
    assert "button.el-button--text" in selectors["list_draft_no"]


def test_attach_upload_clicks_yingdao_link() -> None:
    selectors = __import__("json").loads((FLOW_DIR / "selectors.json").read_text(encoding="utf-8"))
    assert "el-table__fixed-body-wrapper" in selectors["attach_fixed_row"]
    assert "上传文件" in selectors["attach_upload"]
    assert "elBtnA" in selectors["attach_upload"]
    assert "attachment-upload" in selectors["attach_file"]


def test_is_data_blob_waits_for_real_rows() -> None:
    assert flow_module.is_data_blob("") is False
    assert flow_module.is_data_blob("暂无数据") is False
    assert flow_module.is_data_blob("   ") is False
    assert (
        flow_module.is_data_blob("1 9100069442 00010 47-7001373", "9100069442", "47-7001373")
        is True
    )
    assert flow_module.is_data_blob("1 9100069442 00010", "9100069442", "47-7001373") is False
...
def test_header_and_line_diffs() -> None:
    ...
    assert (
        flow_module.line_diffs(
            [{"poNum": "P1", "itemNum": "M1", "netWeight": "3.52100000000000000"}],
            [{"poNum": "P1", "itemNum": "M1", "netWeight": "3.521"}],
        )
        == []
    )
...
def test_plan_attach_uploads_fixed_then_add_dual() -> None:
    plan = flow_module.plan_attach_uploads(
        [
            {"type": "双签PO/协议", "filePath": "d.pdf"},
            {"type": "发票", "filePath": "i.pdf"},
            {"type": "箱单", "filePath": "p.pdf"},
            {"type": "提单", "filePath": "b.pdf"},
            {"type": "双签PO/协议", "filePath": "d2.pdf"},
        ]
    )
    assert [(item["type"], item["mode"], item.get("index")) for item in plan] == [
        ("箱单", "fixed", 0),
        ("发票", "fixed", 1),
        ("提运单", "fixed", 2),
        ("双签PO/协议", "add", None),
