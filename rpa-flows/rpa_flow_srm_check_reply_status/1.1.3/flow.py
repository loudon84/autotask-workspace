...
_READ_REPLY_STATUS_JS = r"""(known) => {
  ...
  for (const item of document.querySelectorAll(
    '.el-form-item, .el-descriptions-item, .detail-item, .el-row, li, p, div'
  )) {
    if (!isVisible(item)) continue;
    const label = clean(
      item.querySelector('.el-form-item__label, .el-descriptions-item__label, .label')?.innerText
    );
    const blob = clean(item.innerText);
    if (label === '回复状态' || blob.startsWith('回复状态')) {
      const hit = fromBlob(blob);
      if (hit) return hit;
    }
  }
  for (const el of document.querySelectorAll('span, div, p, td, label, .el-tag, .el-link')) {
...
class SupplierPortalReplyStatusAdapter:
    ...
    async def reply_status(self):
        known = ("已回签", "待回签", "待签章")
        view_sign = self.selectors.get("view_sign")
        if isinstance(view_sign, str) and view_sign:
            try:
                button = self.page.locator(view_sign)
                if await button.count() and await button.first.is_visible():
                    return "已回签"
            except Exception:
                pass
        try:
            found = await self.page.evaluate(_READ_REPLY_STATUS_JS, list(known))
            if found in known:
                return found
        except Exception:
            pass
        for status in known:
            try:
                loc = self.page.get_by_text(status, exact=True)
