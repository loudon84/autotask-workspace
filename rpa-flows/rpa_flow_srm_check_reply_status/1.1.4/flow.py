...
class SupplierPortalReplyStatusAdapter:
    ...
    async def _row_visible(self, po_no):
    ...
    async def open_order_detail(self, po_no):
        ...
        if not await self._row_visible(po_no):
            self._reply_status = UNSIGNED_REPLY_STATUS
            await self.ctx.events.emit(
                "STEP_SUCCEEDED",
                message="Customer purchase order is not signed back",
                payload={"stepId": step_id, "poNo": po_no, "replyStatus": self._reply_status},
            )
            return
        try:
        ...
        self._reply_status = SIGNED_REPLY_STATUS
        await self.ctx.events.emit(
            "STEP_SUCCEEDED",
    ...
    async def reply_status(self):
        if self._reply_status in {SIGNED_REPLY_STATUS, UNSIGNED_REPLY_STATUS, "待签章"}:
