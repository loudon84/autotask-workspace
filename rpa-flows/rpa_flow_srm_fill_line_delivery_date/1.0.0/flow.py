...
class SupplierPortalFillLineAdapter:
    ...
    async def fill_and_save_line(self, line_no, expected_date):
        ...
        save_line = self.page.locator(self.selector("save_line", line_no=line_no))
        save_all = self.page.locator(self.selector("save_all"))
        try:
            if await save_line.is_visible() and await save_line.is_enabled():
                await save_line.click(timeout=10000)
            elif await save_all.is_visible() and await save_all.is_enabled():
    ...
    async def verify_persisted(self, po_no, line_no, expected_date):
        try:
            ...
            await self.page.locator(self.selector("lines_table")).wait_for(state="visible", timeout=15000)
            lines = await self.collect_order_lines()
            target = find_line(lines, line_no)
            persisted = _clean(target.get("currentExpectedDate"))
            if persisted != expected_date:
                raise RpaHumanRequiredError(
                    "ORDER_LINE_SAVE_UNCONFIRMED",
                    "Saved delivery date was not persisted on the portal",
                    details={"lineNo": line_no, "persistedDate": persisted},
                )
            return persisted
            ...
            raise RpaHumanRequiredError(
                "ORDER_LINE_SAVE_UNCONFIRMED",
                "Saved delivery date requires manual verification",
            ) from None
        except Exception as exc:
            raise RpaHumanRequiredError(
                "ORDER_LINE_SAVE_UNCONFIRMED",
...
async def run(ctx):
    ...
    if _clean(target.get("currentExpectedDate")) == expected_date:
        await _safe_emit(
            ctx,
            "STEP_SUCCEEDED",
            message="Order line already has the requested delivery date",
            payload={"stepId": "srm.idempotency", "poNo": po_no, "lineNo": line_no},
        )
        return {
    ...
    try:
        ...
        persisted = await adapter.verify_persisted(po_no, line_no, expected_date)
    ...
    await _safe_emit(
        ctx,
        "STEP_SUCCEEDED",
        message="Order line delivery date saved and verified",
    ...
    return {
        ...
        "poNo": po_no,
        "lineNumber": line_no,
        "expectedDeliveryDate": persisted,
        "saved": True,
        "idempotent": False,
    }
