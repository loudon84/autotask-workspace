...
class SupplierPortalFillLineAdapter:
    ...
    async def _pick_locator(self, primary_name, fallback_name, **values):
        primary = self.page.locator(self.selector(primary_name, **values)).first
        try:
            if await primary.count() > 0 and await primary.is_visible():
                return primary
        except Exception:
            pass
        return self.page.locator(self.selector(fallback_name, **values)).first

    def date_input(self, line_no):
        # 兼容旧调用；实际填写走 _pick_locator，避免 union+.first 点到 body 克隆。
    ...
    async def fill_and_save_line(self, line_no, expected_date):
        field = await self._pick_locator("expected_date", "expected_date_fallback", line_no=line_no)
        try:
            ...
            if actual != expected_date:
                # Element UI 日期框偶发 input_value 空，但 DOM value 已写入；再读一次。
                actual = _clean(
                    await self.page.evaluate(
                        ...
                        self.selector("expected_date", line_no=line_no),
                    )
                    or await self.page.evaluate(
                        ...
                        self.selector("expected_date_fallback", line_no=line_no),
                    )
                )
            if actual != expected_date:
                raise RpaRetryableError(
        ...
        save_line = await self._pick_locator("save_line", "save_line_fallback", line_no=line_no)
