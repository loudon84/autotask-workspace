...
SPEC = importlib.util.spec_from_file_location(
    "srm_scan_pending_orders_flow_1_1_2",
    FLOW_DIR / "flow.py",
)
...
resolve_drill_po = flow_module.resolve_drill_po
...
class ResolveDrillPoTests(unittest.TestCase):
    def test_default_is_official_sample(self):
        self.assertEqual(resolve_drill_po(None), "POJS2607170008")
        self.assertEqual(resolve_drill_po({}), "POJS2607170008")

    def test_override(self):
        self.assertEqual(resolve_drill_po({"assumedPendingPo": "pojs1"}), "POJS1")

    def test_empty_disables(self):
        self.assertIsNone(resolve_drill_po({"assumedPendingPo": ""}))
