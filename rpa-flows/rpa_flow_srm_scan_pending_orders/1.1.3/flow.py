...
def _pending_only_search():
    return [{"replyStatus": PENDING_REPLY_STATUS}]


def normalize_searches(raw):
    """Binding searches → [{replyStatus?, poNo?, treatAsPending?}]. 空则只查待签章。"""
    if not isinstance(raw, list) or not raw:
        return _pending_only_search()
    searches = []
    for item in raw:
        if not isinstance(item, Mapping):
            continue
        spec = {}
        reply = _clean(item.get("replyStatus") or item.get("reply_status"))
        po_no = _clean(item.get("poNo") or item.get("po_no")).upper()
        if reply:
            spec["replyStatus"] = reply
        if po_no:
            spec["poNo"] = po_no
        if item.get("treatAsPending") or item.get("treat_as_pending"):
            spec["treatAsPending"] = True
        if spec:
            searches.append(spec)
    return searches or _pending_only_search()


def resolve_searches(config=None, raw_input=None):
    """搜索条件只来自 Binding config.searches。没有则只查待签章，不默认样例单号。"""
    sources = [item for item in (config, raw_input) if isinstance(item, Mapping)]
    for source in sources:
        if "searches" in source:
            return normalize_searches(source.get("searches"))
    for source in sources:
        if "assumedPendingPo" not in source and "assumed_pending_po" not in source:
            continue
        po_no = _clean(source.get("assumedPendingPo") or source.get("assumed_pending_po")).upper()
        searches = _pending_only_search()
        if po_no:
            searches.append({"poNo": po_no, "treatAsPending": True})
        return searches
    return _pending_only_search()
...
class SupplierPortalScanAdapter:
    ...
    async def apply_search(self, spec):
        reply = _clean(spec.get("replyStatus"))
        po_no = _clean(spec.get("poNo")).upper()
        if reply:
            await self.select_form_option("回复状态", reply)
        if po_no:
            await self.fill_form_input("订单编号", po_no)
        await self.click_search()

    async def search_pending_signature(self):
        await self.apply_search({"replyStatus": PENDING_REPLY_STATUS})
...
async def run(ctx):
    ...
    raw_config = ctx.config if isinstance(getattr(ctx, "config", None), Mapping) else {}
    searches = resolve_searches(raw_config, raw_input)
    ...
    await ctx.events.emit(
        ...
        payload={"stepId": step_id, "stepType": step_id, "searches": searches},
    )
    ...
    for index, spec in enumerate(searches):
        if index > 0:
            await adapter.click_reset()
        await adapter.apply_search(spec)
        await _safe_screenshot(
            ctx,
            "supplier-portal-pending-orders" if index == 0 else "supplier-portal-drill-po",
            step_id,
        )
        if await adapter.result_is_empty():
            continue
        content = await adapter.export_xlsx(
            "pending-orders.xlsx" if index == 0 else "drill-order.xlsx"
        )
        if not content:
            raise RpaRetryableError(
                "ORDER_LIST_EXPORT_UNAVAILABLE",
                "Supplier portal export is disabled for the current search",
            )
        rows = read_xlsx_table(content)
        treat_as_pending = bool(spec.get("treatAsPending"))
        orders = orders_from_export_rows(rows, treat_as_pending=treat_as_pending)
        if not orders:
            continue
        parts = []
        if spec.get("replyStatus"):
            parts.append(f"replyStatus={spec['replyStatus']}")
        if spec.get("poNo"):
            parts.append(f"订单编号={spec['poNo']}")
        source_filter = " ".join(parts) or source_filter
        if treat_as_pending:
            drill = {
                "assumedPending": True,
                "poNo": spec.get("poNo") or (orders[0]["poNo"] if orders else ""),
                "note": "演练 Binding treatAsPending：按订单编号导出后当成待签章扫入",
            }
        break
