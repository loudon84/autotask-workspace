...
操作：入库确认时间面板 → 对账状态=未提交 → 查询 → 表头全选 →
核对「共 N 条」与 lines 数量 → 定位「生成对账单」。
...
READ_SELECTED_COUNT_JS = r"""() => {
  ...
  for (const el of nodes) {
    ...
    const match = text.match(/^已选择\s*(\d+)\s*项/);
...
CLICK_SELECT_ALL_JS = r"""() => {
  ...
  const candidates = [
    ...table.querySelectorAll(
      '.el-table__fixed-left thead th.el-table-column--selection .el-checkbox__inner'
    ),
    ...table.querySelectorAll(
      '.el-table__fixed thead th.el-table-column--selection .el-checkbox__inner'
    ),
    ...table.querySelectorAll(
    ...
    async def select_all_and_assert_count(self, expected):
        ...
        if result not in ("ok", "already"):
            ...
            raise RpaBusinessError(
                ...
                f"receipt list select-all is unavailable ({result})",
            )
