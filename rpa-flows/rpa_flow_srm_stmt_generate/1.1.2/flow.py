...
操作：入库确认时间面板 → 对账状态=未提交 → 查询 → 点表头 checkbox →
「共 N 条」和「已选择 M 项」都必须等于 lines 数量 → 定位「生成对账单」。
不逐行勾、不去搜「全选」文字、不改每页条数、不翻页。
...
READ_SELECTED_COUNT_JS = r"""() => {
  const nodes = [...document.querySelectorAll('span, div, p, label, button, a')];
  let best = null;
  let bestLen = Infinity;
  for (const el of nodes) {
    const text = String(el.innerText || '').replace(/\s+/g, ' ').trim();
    const match = text.match(/已选择\s*(\d+)\s*项/) || text.match(/已选\s*(\d+)\s*条/);
    if (!match) continue;
...
class ReceiptListAdapter:
    ...
    async def select_all_and_assert_count(self, expected):
        ...
        selected = await self.page.evaluate(READ_SELECTED_COUNT_JS)
        if not isinstance(selected, int):
            await _safe_screenshot(self.ctx, "stmt-generate-selected-count-missing", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_SELECTED_COUNT_MISSING",
                "selected count is unavailable after header checkbox",
            )
        if selected != expected:
            await _safe_screenshot(self.ctx, "stmt-generate-count-mismatch", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_COUNT_MISMATCH",
                f"selected count {selected} != selected lines {expected}",
            )
