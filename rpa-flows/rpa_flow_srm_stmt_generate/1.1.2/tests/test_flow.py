...
class SelectAllCountTests(unittest.IsolatedAsyncioTestCase):
    async def test_select_all_when_total_and_selected_match(self):
        page = FakePage({}, evaluate_results=["共 26 条", "ok", 26])
    ...
    async def test_rejects_when_selected_count_missing(self):
        page = FakePage({}, evaluate_results=["共 26 条", "ok", None])
        with self.assertRaises(RpaBusinessError) as raised:
            await _adapter(page).select_all_and_assert_count(26)
        self.assertEqual(raised.exception.code, "SRM_STMT_GENERATE_SELECTED_COUNT_MISSING")

    async def test_rejects_when_select_all_only_covers_current_page(self):
        page = FakePage({}, evaluate_results=["共 26 条", "ok", 10])
        with self.assertRaises(RpaBusinessError) as raised:
            await _adapter(page).select_all_and_assert_count(26)
        self.assertEqual(raised.exception.code, "SRM_STMT_GENERATE_COUNT_MISMATCH")
