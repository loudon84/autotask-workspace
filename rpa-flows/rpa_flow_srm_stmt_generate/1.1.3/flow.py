...
操作：入库确认时间面板 → 对账状态=未提交 → 查询 → 每页 100 条 →
核「共 N 条」= lines 数量 → 点表头 checkbox；N>100 则翻页再点表头。
不逐行勾、不读「已选择」。
...
READ_LIST_TOTAL_JS = r"""() => {
  const total = document.querySelector('.el-pagination__total');
  return total ? String(total.innerText || '').trim() : '';
}"""

CLICK_SELECT_ALL_JS = r"""() => {
  ...
  const candidates = [
    ...table.querySelectorAll(
      '.el-table__header-wrapper thead th.el-table-column--selection .el-checkbox__inner'
    ),
    ...table.querySelectorAll('thead th.el-table-column--selection .el-checkbox__inner'),
  ];
  const box = candidates.find(visible);
  if (!box) return 'checkbox-missing';
  const wrap = box.closest('.el-checkbox');
  if (wrap && wrap.classList.contains('is-checked')) return 'already';
  box.click();
  return 'ok';
}"""
...
def parse_pagination_total(text):
    cleaned = _clean(text).replace(",", "")
    match = re.search(r"共\s*(\d+)\s*条", cleaned)
    if match:
...
class ReceiptListAdapter:
    ...
    async def set_page_size_100(self):
        sizes = self.page.locator(self.selector("page_sizes"))
        if await sizes.count() == 0:
            return False
        current = _clean(await sizes.inner_text())
        if re.search(r"\b100\b", current):
            return True
        trigger = sizes.locator(".el-select, .el-input, input").first
        try:
            await trigger.click(timeout=4000)
        except Exception as exc:
            await _safe_screenshot(self.ctx, "stmt-generate-page-size-missing", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_PAGE_SIZE_MISSING",
                "receipt list page size control is not clickable",
            ) from exc
        await self.page.wait_for_timeout(200)
        option = self.page.locator(self.selector("page_size_100")).first
        try:
            await option.wait_for(state="visible", timeout=4000)
            await option.click(timeout=4000)
        except Exception as exc:
            await _safe_screenshot(self.ctx, "stmt-generate-page-size-100-missing", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_PAGE_SIZE_MISSING",
                "receipt list page size 100 is unavailable",
            ) from exc
        await self._wait_loading_done()
        await self.page.wait_for_timeout(300)
        return True

    async def click_header_select_all(self):
        result = await self.page.evaluate(CLICK_SELECT_ALL_JS)
        if result not in ("ok", "already"):
            await _safe_screenshot(self.ctx, "stmt-generate-select-all-missing", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_SELECT_ALL_MISSING",
                f"receipt list header checkbox is unavailable ({result})",
            )
        await self.page.wait_for_timeout(200)

    async def go_next_page(self):
        button = self.page.locator(self.selector("next_page")).first
        ...
        if disabled or "disabled" in classes.split() or "is-disabled" in classes:
            return False
    ...
    async def select_all_and_assert_count(self, expected):
        # @lat: [[domain#StatementBill#Statement Generate Select All]]
        total_text = await self.page.evaluate(READ_LIST_TOTAL_JS)
        total = parse_pagination_total(total_text)
        if total is None:
            await _safe_screenshot(self.ctx, "stmt-generate-count-missing", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_COUNT_MISSING",
                "receipt list total is unavailable",
            )
        if total != expected:
            await _safe_screenshot(self.ctx, "stmt-generate-count-mismatch", "srm.generate")
            raise RpaBusinessError(
                "SRM_STMT_GENERATE_COUNT_MISMATCH",
                f"receipt list total {total} != selected lines {expected}",
            )
        size_set = await self.set_page_size_100()
        await self.click_header_select_all()
        if expected <= PAGE_SIZE and size_set:
            return
        pages = 1
        while pages < MAX_PAGES:
            moved = await self.go_next_page()
            if not moved:
                return
            await self.click_header_select_all()
            pages += 1
        await _safe_screenshot(self.ctx, "stmt-generate-too-many-pages", "srm.generate")
        raise RpaBusinessError(
            "SRM_STMT_GENERATE_TOO_MANY_PAGES",
            f"receipt list exceeded {MAX_PAGES} pages",
        )
...
async def run(ctx):
    ...
    await _safe_emit(
        ctx,
        "STEP_STARTED",
        message="Selecting all receipt lines and locating generate button",
        payload={"stepId": step_id, "stepType": step_id, "dryRun": dry_run},
    )
    await adapter.select_all_and_assert_count(len(lines))
