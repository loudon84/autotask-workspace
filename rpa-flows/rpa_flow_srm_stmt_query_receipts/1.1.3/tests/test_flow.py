...
SPEC = importlib.util.spec_from_file_location(
    "srm_stmt_query_receipts_flow_1_1_3",
    FLOW_DIR / "flow.py",
)
...
inbound_range_datetimes = flow_module.inbound_range_datetimes
applied_range_is_valid = flow_module.applied_range_is_valid
...
class InboundRangeDatetimeTests(unittest.TestCase):
    def test_same_day_starts_midnight_ends_last_second(self):
        start = parse_ymd("2026-08-01")
        end = parse_ymd("2026-08-01")
        self.assertEqual(
            inbound_range_datetimes(start, end),
            ("2026-08-01 00:00:00", "2026-08-01 23:59:59"),
        )

    def test_rejects_both_end_of_day(self):
        start = parse_ymd("2026-08-01")
        end = parse_ymd("2026-08-01")
        self.assertFalse(
            applied_range_is_valid(
                "2026-08-01 23:59:59",
                "2026-08-01 23:59:59",
                start,
                end,
    ...
    def test_accepts_official_same_day_range(self):
        start = parse_ymd("2026-08-01")
        end = parse_ymd("2026-08-01")
        self.assertTrue(
            applied_range_is_valid(
