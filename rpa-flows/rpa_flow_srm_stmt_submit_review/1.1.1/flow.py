...
dryRun=true：扫描真做；按钮可见可点后截图，不 click。
dryRun=false：点击提交审核。
"""
...
CLICK_PAYABLE_JS = r"""({ rowIndex, buttonText }) => {
  ...
  for (const source of sources) {
    const rows = [...source.querySelectorAll('.el-table__body-wrapper tbody tr')];
    const row = rows[rowIndex];
    if (!row) continue;
    const btn = [...row.querySelectorAll('button, a, span, .el-button')].find((el) => {
      return visible(el) && String(el.innerText || '').includes(buttonText);
    });
    if (btn) {
      btn.click();
      return 'ok';
...
async def run(ctx):
    ...
    if dry_run:
        await _safe_emit(
            ctx,
            "STEP_SUCCEEDED",
            message="Submit button found; dry-run skipped the click",
            payload={
                "stepId": step_id,
                "dryRun": True,
