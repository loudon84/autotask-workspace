...
操作：对账列表 → 按日期+金额找未对账 → 收货应付 → 扫描发票 → 定位「提交审核」
dryRun=true：扫描真做；对「提交审核」做 Playwright trial click（可点性检查，不真点）。
dryRun=false：点击提交审核。
"""
...
from nodeskclaw_rpa_engine.runtime.actionability import assert_clickable
...
READ_BASE_INFO_JS = r"""() => {
  ...
  const firstVisible = (selector) => [...document.querySelectorAll(selector)].find(isVisible);
  const root = firstVisible('.el-drawer') || firstVisible('.el-dialog') || document.querySelector('.app-container') || document.body;
...
def require_expected_invoice(raw_input):
    expected_no = normalize_invoice_no(
        raw_input.get("expectedInvoiceNo") or raw_input.get("expected_invoice_no")
    )
    expected_amount = normalize_invoice_amount(
        raw_input.get("expectedInvoiceAmount") or raw_input.get("expected_invoice_amount")
    )
    if not expected_no or not expected_amount:
        raise RpaFatalError(
            "STMT_EXPECTED_INVOICE_REQUIRED",
            "expectedInvoiceNo/expectedInvoiceAmount are required",
        )
    return expected_no, expected_amount


def _amount_key(raw):
    text = normalize_invoice_amount(raw)
    if not text:
        return None
    try:
...
def assert_invoice_matches(expected_no, expected_amount, actual_no, actual_amount):
    got_no = normalize_invoice_no(actual_no)
    want_no = normalize_invoice_no(expected_no)
    got_amt = _amount_key(actual_amount)
    want_amt = _amount_key(expected_amount)
    if got_no == want_no and got_amt is not None and got_amt == want_amt:
        return
    raise RpaBusinessError(
        "STMT_INVOICE_MISMATCH",
        "二次扫描与已核对发票不一致："
        f"页面 {want_no}/{expected_amount}，本次 {got_no}/{actual_amount}",
    )
...
def submit_result(*, dry_run, check_date, check_amount, invoice_no, invoice_amount, file_count, button_found=True):
    payload = {
        ...
        "dryRun": bool(dry_run),
        "submitButtonFound": bool(button_found),
    }
    if dry_run:
        payload["blockedAction"] = "submit_review"
        payload["checkStatus"] = "未对账"
        payload["invoiceStatus"] = "未上传"
    else:
        payload["checkStatus"] = "已对账"
        payload["invoiceStatus"] = "审核中"
    return payload
...
class StatementSubmitAdapter:
    ...
    async def _dismiss_blocking_layers(self):
        ...
        try:
            if await cancel.count():
                await cancel.first.click(timeout=1500)
        except Exception:
            pass
    ...
    async def select_form_option(self, label, option):
        ...
        if await item.count() == 0:
            return False
        trigger = item.locator(".el-select input, .el-select .el-input__inner").first
    ...
    async def open_statement_list(self):
        ...
        on_payable = await self.page.locator(
            "button:has-text('扫描发票'):visible, button:has-text('提交审核'):visible"
        ).count()
        if on_payable:
            await self.page.goto(f"{portal_root}/#/", wait_until="domcontentloaded")
            await self.page.wait_for_timeout(400)
    ...
    async def _click_upload_confirm(self):
        step_id = "srm.upload_invoice"
        confirm = self.page.locator(self.selector("upload_confirm")).locator("visible=true").first
        try:
            await confirm.wait_for(state="visible", timeout=8000)
        except Exception as exc:
            await _safe_screenshot(self.ctx, "stmt-upload-confirm-missing", step_id)
            raise RpaRetryableError(
                "SRM_STMT_UPLOAD_CONFIRM_MISSING",
                "Upload confirm button is not visible",
            ) from exc
        for _ in range(20):
            disabled = False
            try:
                disabled = await confirm.is_disabled()
            except Exception:
                disabled = False
            classes = _clean(await confirm.get_attribute("class"))
            if not disabled and "is-disabled" not in classes:
                break
            await self.page.wait_for_timeout(250)
        else:
            await _safe_screenshot(self.ctx, "stmt-upload-confirm-disabled", step_id)
            raise RpaBusinessError(
                "SRM_STMT_UPLOAD_CONFIRM_DISABLED",
                "Upload confirm button is visible but disabled",
            )
        try:
            ...
            try:
                ...
                await _safe_screenshot(self.ctx, "stmt-upload-confirm-unclickable", step_id)
                raise RpaRetryableError(
                    "SRM_STMT_UPLOAD_CONFIRM_UNCLICKABLE",
                    "Could not click upload confirm",
                ) from exc
        await _safe_emit(
            self.ctx,
            "STEP_SUCCEEDED",
            message="Clicked upload confirm",
            payload={"stepId": step_id},
        )
        try:
    ...
    async def scan_invoices(self, file_paths):
        ...
        await self.page.wait_for_timeout(400)
        await self._click_upload_confirm()
        invoice_no = ""
        invoice_amount = ""
        for _ in range(30):
            try:
                ...
                await _safe_screenshot(self.ctx, "stmt-invoice-read-failed", step_id)
                raise RpaRetryableError(
                    "SRM_STMT_INVOICE_READ_FAILED",
                    "Could not read invoice fields from payable page",
                ) from exc
    ...
    async def locate_submit_button(self):
        button = self.page.locator(self.selector("submit_button")).locator("visible=true").first
        try:
        ...
        try:
            report = await assert_clickable(
                button,
                name="提交审核",
                error_code="SRM_STMT_SUBMIT_UNCLICKABLE",
            )
        except RpaBusinessError:
            await _safe_screenshot(self.ctx, "stmt-submit-button-unclickable", "srm.submit_review")
            raise
...
async def run(ctx):
    ...
    expected_no, expected_amount = require_expected_invoice(raw_input)
    dry_run = is_dry_run(ctx)

    await ctx.log.info("Starting statement submit review Flow", {"dryRun": dry_run})
    adapter = StatementSubmitAdapter(ctx)
    await adapter.login()
    await adapter.open_statement_list()
    await adapter.open_payable(check_date, check_amount)

    step_id = "srm.submit_review"
    await _safe_emit(
        ctx,
        "STEP_STARTED",
        message="Scanning invoices and locating submit button",
    ...
    assert_invoice_matches(expected_no, expected_amount, invoice_no, invoice_amount)
    await install_write_guard(ctx.page, dry_run=dry_run, allow_upload=True)
    button, clickable = await adapter.locate_submit_button()
    await _safe_screenshot(ctx, "stmt-submit-button", step_id)
    if dry_run:
        await _safe_emit(
            ctx,
            "STEP_SUCCEEDED",
            message="Submit button trial-click passed; dry-run skipped the real click",
            payload={
                "stepId": step_id,
                "dryRun": True,
                ...
                "invoiceNo": invoice_no,
                "trialOk": True,
                "hitsSelf": clickable.get("hitsSelf"),
            },
        )
    ...
    await adapter.click_submit(button)
    await _safe_emit(
        ctx,
        "STEP_SUCCEEDED",
        message="Statement submitted for review",
        payload={"stepId": step_id, "invoiceNo": invoice_no},
    )
    return submit_result(
        dry_run=False,
