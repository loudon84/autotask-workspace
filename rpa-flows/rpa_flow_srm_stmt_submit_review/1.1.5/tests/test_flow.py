...
SPEC = importlib.util.spec_from_file_location(
    "srm_stmt_submit_review_flow_1_1_5",
    FLOW_DIR / "flow.py",
)
...
class FakeLocator:
    ...
    async def evaluate(self, _js):
        return {
            "width": 80,
            "height": 28,
            "disabledAttr": self.disabled,
            "classDisabled": self.disabled,
            "pointerEvents": "none" if self.disabled else "auto",
            "hitsSelf": not self.disabled,
        }

    async def click(self, timeout=0, trial=False, **kwargs):
        if trial:
            if self.disabled:
                raise TimeoutError("element is not enabled")
            if self.page is not None:
                self.page.trials.append(self.selector)
            return
        if self.page is not None:
...
class LocateSubmitButtonTests(unittest.IsolatedAsyncioTestCase):
    async def test_returns_visible_enabled_button_without_clicking(self):
        ...
        button, report = await adapter.locate_submit_button()
        self.assertTrue(button.visible)
        self.assertTrue(report["trialOk"])
        self.assertEqual(page.clicks, [])
        self.assertTrue(page.trials)

    async def test_disabled_button_fails_trial_without_clicking(self):
        page = FakePage(
            {
                SELECTORS["submit_button"]: FakeLocator(visible=True, disabled=True),
            }
        )
        adapter = flow_module.StatementSubmitAdapter(
            SimpleNamespace(
        ...
        with self.assertRaises(RpaBusinessError) as caught:
            await adapter.locate_submit_button()
        self.assertEqual(caught.exception.code, "SRM_STMT_SUBMIT_UNCLICKABLE")
        self.assertEqual(page.clicks, [])


class OfficialPackageGuardTests(unittest.TestCase):
    def test_manifest_is_1_1_5(self):
    ...
    def test_uses_official_route_and_submit_button(self):
        ...
        self.assertIn("assert_clickable", raw)
        self.assertIn("trial-click", raw)
        ...
        self.assertNotIn(":visible", flow_module.READ_BASE_INFO_JS)
        self.assertIn("firstVisible", flow_module.READ_BASE_INFO_JS)
        self.assertIn("_click_upload_confirm", raw)
        self.assertIn("SRM_STMT_UPLOAD_CONFIRM_MISSING", raw)
        self.assertIn("_dismiss_blocking_layers", raw)
        run_src = raw.split("async def run")[-1]
        self.assertLess(run_src.find("scan_invoices"), run_src.find("install_write_guard"))
