...
async def _login(ctx):
    ...
    page = ctx.page
    try:
        already = page.locator(selectors["login_success"])
        captcha_now = page.locator(selectors["captcha_image"])
    ...
    captcha = page.locator(selectors["captcha_image"])
    success = page.locator(selectors["login_success"])
    try:
        for _ in range(50):
    ...
    await page.fill(selectors["captcha"], code)
    agreement = page.locator(selectors["agreement"])
