...
> **适用门户：演示门户**（`http://192.168.102.247:3000`，带 `data-rpa` 标记）。本包选择器全部基于 `data-rpa`，**不能用于正式门户**（`https://supplier.tiandy.com`，无 `data-rpa`）。正式门户需单独出一个版本（选择器从正式站探测）。绑定时按门户环境选对应版本；同一门户环境内始终用最新版本。
