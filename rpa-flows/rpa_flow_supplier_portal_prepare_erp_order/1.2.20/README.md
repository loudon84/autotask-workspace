...
> **适用门户：正式门户**（`https://supplier.tiandy.com`，无 `data-rpa`）。本包选择器全部为正式站文案/Element 选择器，**不能用于演示门户**。演示站继续用 `1.2.11`。绑定时按门户环境选对应版本。建 SDMS 写的是我们自己的 ERP，Binding **不要**设 `dryRun`。

本 Flow 使用 Engine 托管的 Playwright 页面登录供应商门户，按任务输入的采购订单号进入详情页，下载 XLSX 订单附件，以门户详情的“订单行号 + 客户料号”核对附件明细，核对成功后才构造并提交 ERP 销售订单。

> 写操作警告：本版本会产生外部 ERP 写入。发布不等于启用；未经单独授权不得切换 WorkflowBinding 或运行任务。ERP 提交结果不明确时不得自动重试。正式 SRM 只读：本包只下载订单，不点保存/签章/提交。

## 1.2.20 变更（相对 1.2.19）

ERP 销售订单行表不再传 `unTaxPrice`，也不再倒算未税价。

## 1.2.19 变更（相对 1.2.18）

ERP 销售订单行表不再传 `taxRate`。未税价仍按默认税率 0.13 从含税单价倒算，仅写入 `unTaxPrice`。

## 1.2.18 变更（相对 1.2.17）

`ErpSalesOrderClient.__aexit__` 不再 `del exc` 后再读。1.2.17 导入被 ERP 拒（如缺价目表）时会把真实业务错误变成 `FLOW_UNHANDLED_ERROR` / 「Flow execution failed」。

## 1.2.17 变更（相对 1.2.16）

ERP HTTP 改走 `ctx.http`，token + 导入各记一行接口调用日志。业务逻辑与 1.2.16 相同。

## 1.2.16 变更（相对 1.2.15）

SDMS 销售订单主表补两个字段，都从门户租约读取，不再留空、不写死：

- `customerSubCode` ← 门户客户代码 `erpEntityCode`（租约 `customerCode`），如 `C007193-01`
- `orgCode` ← 门户我方公司编号 `ou`（租约 `ou`），如 `104`

缺任一字段在调 ERP 前失败（`ERP_REQUIRED_FIELD_MISSING`）。`orgName` 仍来自门户业务实体。

## 1.2.15 变更（相对 1.2.14）

Element UI 冻结「操作」列后，主表体里的「详情」是隐藏副本。`force=True` 点到它会报成功，页面仍停在订单列表，随后空等「导出订单明细」。改为先按订单号对齐行号，再点 `.el-table__fixed-right` 里真正可见的「详情」，不用 force。

## 1.2.14 变更（相对 1.2.13）

正式详情附件入口是「导出订单明细」，不是演示站的「下载订单」。打开详情的确认和下载都改认这四个字。

## 1.2.13 变更（相对 1.2.12）

打开详情改为点该行「详情」（`force` 点击），并用附件按钮确认已进详情。避免列表页已有订单号时误判，也避免默认 click 被遮罩挡住空等到超时。

## 1.2.12 变更（相对 1.2.11）

正式门户专用包：登录走 `login_official_srm`（与扫单 1.1.2 相同选择器），列表/详情/下载用正式站文案（查询、详情、下载订单），无 `data-rpa`。下载允许无确认框直接出文件。`orgName` 仍从租约 `businessEntity` 读取。

## 1.2.11 变更（相对 1.2.9）

演示门户专用包：`selectors.json` 全部改为纯 `data-rpa`，去掉 1.2.9 的 `.el-table`/文本兜底。修复 1.2.9 在演示门户的 strict mode 报错（`order_page`/`lines_table` 用逗号 OR 同时匹配「容器 + 容器内表格」两个元素）。其余逻辑与 1.2.9 一致（`orgName` 仍从租约 `businessEntity` 读取）。

## 1.2.9 变更（相对 1.2.8）

ERP 抬头 `orgName` 改从租约 `businessEntity`（门户「业务实体」，我方公司全称）读取，不再用 XLSX `supplierName`。缺业务实体返回 `ERP_REQUIRED_FIELD_MISSING`。`orgCode` / `customerSubCode` 从 1.2.16 起由门户 `ou` / `erpEntityCode` 下发。

## 1.2.8 变更（相对 1.2.7）

ERP 基址、OAuth 客户端和客户名称从租约 `ctx.config` / `ctx.credentials` 读取（Task `.env` 的 `ERP_BASE_URL` 等经租约下发）。源码不再写死测试环境 IP 或密钥。缺基址返回 `ERP_ENDPOINT_NOT_CONFIGURED`，缺密钥返回 `ERP_CREDENTIALS_NOT_CONFIGURED`。

## 1.2.7 变更（相对 1.2.6）

正式站无 `data-rpa`：登录/查单/详情增加表头与文案选择器，并兼容 `#/order/list`。演示站原选择器仍保留。验证码无法按文件名映射时仍回退 `HUMAN_VERIFICATION_REQUIRED`（客服在 Client 填一次）。**建 SDMS 是写入我们自己的 ERP，此包不要开 Binding.dryRun。**
...
ERP OAuth 凭据从租约 `ctx.credentials` 读取，不写在 Flow 源码里。不得把 ZIP 提交到公开仓库、公开制品库或不受控日志；不得输出凭据、Token、Authorization Header、OAuth 查询参数或原始认证响应。