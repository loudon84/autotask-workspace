...
FLOW_SPEC = importlib.util.spec_from_file_location(
    "supplier_portal_prepare_erp_order_flow_1_2_16",
    FLOW_DIR / "flow.py",
)
...
class StubHttp:
    """Test double for ctx.http (IntegrationHttp) backed by MockTransport."""
    ...
    async def request(self, method, url, *, system="UNKNOWN", **kwargs):
...
_customer_sub_code_from_ctx = flow_module._customer_sub_code_from_ctx
_org_code_from_ctx = flow_module._org_code_from_ctx
...
def make_erp_client(**kwargs):
    ...
    values = {
        ...
        "http_client": StubHttp(transport=transport),
    }
...
def run_config(**overrides):
    payload = {
        ...
        "customerCode": CUSTOMER_SUB_CODE,
        "ou": ORG_CODE,
    }
...
class PackageContractTests(unittest.TestCase):
    def test_manifest_and_selectors_describe_1_2_6_reconciliation(self):
        ...
        self.assertEqual(manifest["version"], "1.2.20")
...
class ErpSalesOrderClientTests(unittest.IsolatedAsyncioTestCase):
    def payload(self):
        ...
        payload, _ = build_erp_draft(
            ...
            org_name=ORG_NAME,
            customer_sub_code=CUSTOMER_SUB_CODE,
            org_code=ORG_CODE,
        )
        return payload
    ...
    async def test_run_emits_erp_row_failure_details(self):
        ...
        self.assertEqual(failed[-1]["message"], failed_row["processMessage"])
...
class ErpDraftTests(unittest.TestCase):
    def test_uses_only_xlsx_mapping_and_declared_defaults(self):
        ...
        self.assertEqual(line["unitSellingPrice"], 22.9448)
        self.assertNotIn("taxRate", line)
        self.assertNotIn("unTaxPrice", line)
        self.assertEqual(line["requestDate"], "2026-06-24")
        for name in (
            "priceListName",
            "factoryLocation",
        ...
        self.assertNotIn("taxRate", resolved[0])
        self.assertNotIn("unTaxPrice", resolved[0])
    ...
    def test_org_name_comes_from_portal_business_entity_not_xlsx(self):
        ...
        self.assertEqual(payload[0]["orgCode"], ORG_CODE)
        self.assertEqual(payload[0]["customerSubCode"], CUSTOMER_SUB_CODE)
    ...
    def test_reads_customer_sub_code_and_org_code_from_lease(self):
        ctx = SimpleNamespace(config=run_config(), credentials=run_credentials())
        self.assertEqual(_customer_sub_code_from_ctx(ctx), CUSTOMER_SUB_CODE)
        self.assertEqual(_org_code_from_ctx(ctx), ORG_CODE)
    ...
    def test_missing_business_entity_fails_before_erp(self):
        ...
        with self.assertRaises(RpaBusinessError) as caught:
            _org_name_from_ctx(ctx)
        self.assertEqual(caught.exception.code, "ERP_REQUIRED_FIELD_MISSING")

    def test_missing_customer_code_fails_before_erp(self):
        ctx = SimpleNamespace(
            config=run_config(customerCode=""),
            credentials=run_credentials(),
        )
        with self.assertRaises(RpaBusinessError) as caught:
            _customer_sub_code_from_ctx(ctx)
        self.assertEqual(caught.exception.code, "ERP_REQUIRED_FIELD_MISSING")

    def test_missing_ou_fails_before_erp(self):
        ctx = SimpleNamespace(
            config=run_config(ou=""),
            credentials=run_credentials(),
        )
        with self.assertRaises(RpaBusinessError) as caught:
            _org_code_from_ctx(ctx)
        self.assertEqual(caught.exception.code, "ERP_REQUIRED_FIELD_MISSING")
