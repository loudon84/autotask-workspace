...
class SupplierPortalAdapter:
    ...
    async def collect_order_line_identities(self):
        rows = self.page.locator(self.selector("detail_rows"))
        try:
            await rows.first.wait_for(state="visible", timeout=10000)
            raw_lines = await rows.evaluate_all(
                """
                (items) => items.map((row) => {
                  const cells = [...row.querySelectorAll('td')];
                  return {
                    lineNumber: cells[0]?.innerText ?? '',
                    customerItemNumber: cells[1]?.innerText ?? '',
                  };
                })
                """
            )
    ...
    async def wait_for_detail_stable(self):
        try:
            ...
            detail_rows = self.page.locator(self.selector("detail_rows"))
            await detail_rows.first.wait_for(state="visible", timeout=10000)
            ...
            for attempt in range(20):
                layout = await self.page.evaluate(
                    ...
                    """,
                    {
                        ...
                        "rowSelector": self.selector("detail_rows"),
                    },
                )
