...
def build_erp_draft(po_no, attachment, ordered_date=None, customer_name=None):
...
class SupplierPortalAdapter:
    ...
    async def open_order_detail(self, po_no):
        ...
        detail = self.page.locator(self.selector("order_detail", po_no))
        if await detail.count() == 0:
            detail = row.get_by_text("详情", exact=True).first
        await detail.click()
        try:
            detail_no = self.page.locator(self.selector("detail_po_number", po_no))
            if await detail_no.count():
                await detail_no.wait_for(state="visible", timeout=15000)
            else:
                await self.page.get_by_text(po_no, exact=True).first.wait_for(
    ...
    async def collect_order_line_identities(self):
        ...
        try:
            await self.page.locator(lines_table).wait_for(
                state="visible",
                timeout=10000,
            )
