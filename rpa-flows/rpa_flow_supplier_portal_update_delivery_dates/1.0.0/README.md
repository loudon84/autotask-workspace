...
本 Flow 使用 Engine 托管的 Playwright 页面，登录供应商门户，按订单编号搜索待签章订单，逐订单行填写预计交货日期，保存后完成签章并验证回复状态。
...
非编辑状态下，Flow 仍会从只读表格读取行号、料号和预计交货日期。
...
- 门户凭据由 Portal `credentialRef` 注入 `ctx.credentials`。
