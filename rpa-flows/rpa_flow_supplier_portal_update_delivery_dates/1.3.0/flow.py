...
class SupplierPortalDeliveryDateAdapter:
    ...
    async def sign_and_verify(self, _po_no, lines):
        try:
        ...
        return success_message, SIGNED_REPLY_STATUS, lines
...
def _success_result(
    ...
    lines,
    *,
    verification_mode="PERSISTENCE_VERIFIED",
...
async def run(ctx):
    ...
    try:
        ...
        await _safe_screenshot(
            ctx,
            "supplier-portal-delivery-dates-sign-accepted-demo",
            "srm.sign_order",
        )
    ...
    await _safe_emit(
        ctx,
        "STEP_SUCCEEDED",
        message="Customer purchase order sign action accepted by demo portal",
        payload={
            "stepId": "srm.sign_order",
            "poNo": po_no,
            "lineCount": len(lines),
    ...
    return _success_result(
        po_no,
        lines,
        verification_mode="ACTION_CONFIRMATION_ONLY",
