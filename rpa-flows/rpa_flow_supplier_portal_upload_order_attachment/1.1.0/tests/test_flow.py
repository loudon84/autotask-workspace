...
class InputAndContractTests(unittest.TestCase):
    ...
    def test_manifest_and_selectors_define_frozen_contract(self):
        ...
        self.assertIn(
            "[data-rpa='order-detail-view-sign-btn']",
            selectors["view_sign"],
        )
        self.assertIn("reply_status", selectors)
        source = (FLOW_DIR / "flow.py").read_text(encoding="utf-8")
        self.assertIn("verify_signed", source)
        self.assertIn("ORDER_NOT_SIGNED", source)
        self.assertIn("download_signed_contract", source)
        self.assertIn("SIGNED_CONTRACT_WRONG_FILE", source)
