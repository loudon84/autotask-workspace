...
`username` 必须是当前 AutoTask Auth 登录账号。`filename` 为客户订单号 `po_no` 加下载文件后缀（如 `POJS2607130002.pdf`）。

## 1.2.5 变更

- 修复：上传到 SDMS 的 `filename`（附件名）原为纯 `po_no`（无后缀），SDMS 看不出文件类型。改为 `po_no` + 下载文件后缀（取自 `sourceFileName`），如 `POJS2607130002.pdf`。`file` 多部分字段仍用真实下载文件名。
...
| `filename` | 客户订单号 `po_no` + 下载文件后缀（如 `POJS2607130002.pdf`） |
| `file` | 双方签章合同 |
