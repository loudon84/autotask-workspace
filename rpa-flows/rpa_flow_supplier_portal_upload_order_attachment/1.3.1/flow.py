...
_READ_LIST_REPLY_STATUS_JS = r"""(poNo) => {
  const clean = (v) => String(v || '').replace(/\s+/g, ' ').trim();
  const known = ['已回签', '待回签', '待签章'];
  const tables = [...document.querySelectorAll('.el-table')].filter(
    (table) => !table.closest('.el-table__fixed-right, .el-table__fixed')
  );
  for (const table of tables) {
    const headers = [...table.querySelectorAll('.el-table__header-wrapper th')].map(
      (th) => clean(th.textContent)
    );
    const poIdx = headers.findIndex((h) => h === '订单编号' || h.includes('订单编号'));
    const statusIdx = headers.findIndex((h) => h === '回复状态' || h.includes('回复状态'));
    if (statusIdx < 0) continue;
    const bodies = [...table.querySelectorAll('.el-table__body-wrapper tbody')].filter(
      (body) => !body.closest('.el-table__fixed-right, .el-table__fixed')
    );
    for (const body of bodies) {
      for (const row of body.querySelectorAll(':scope > tr')) {
        ...
        const poText = poIdx >= 0 ? clean(cells[poIdx]?.textContent) : blob;
        if (!poText.includes(poNo) && !blob.includes(poNo)) continue;
        const status = clean(cells[statusIdx]?.textContent);
        if (known.includes(status)) return status;
        for (const item of known) {
          if (blob.includes(item)) return item;
        }
      }
    }
  }
...
class SupplierPortalAttachmentAdapter:
    ...
    async def open_order_detail(self, po_no):
        ...
        self._list_reply_status = await self._read_list_reply_status(po_no)
        try:
        ...
        await self._wait_loading_done()
        try:
            await self.page.locator(self.selector("view_sign")).first.wait_for(
                state="visible",
                timeout=15000,
            )
        except Exception as exc:
    ...
    async def _read_list_reply_status(self, po_no):
        known = (SIGNED_REPLY_STATUS, "待回签", "待签章")
        try:
            found = await self.page.evaluate(_READ_LIST_REPLY_STATUS_JS, po_no)
            if found in known:
                return found
        except Exception:
            pass
        row = self.page.locator(".el-table__body-wrapper tbody tr").filter(has_text=po_no)
        for status in known:
            try:
                loc = row.get_by_text(status, exact=True)
    ...
    async def reply_status(self):
        if self._list_reply_status:
            return self._list_reply_status
        return ""
    ...
    async def verify_signed(self):
        ...
        if not await self._is_visible(view_sign):
            raise RpaBusinessError(
    ...
    async def download_signed_contract(self):
        ...
        try:
            ...
            async with self.page.expect_download(timeout=20000) as info:
                await self.page.locator(self.selector("view_sign")).first.click(
                    timeout=8000
                )
...
async def run(ctx):
    ...
    source_suffix = Path(attachment["sourceFileName"]).suffix
    attachment_name = f"{po_no}{source_suffix}"
