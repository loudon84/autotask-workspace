...
SPEC = importlib.util.spec_from_file_location(
    "supplier_portal_upload_order_attachment_flow_1_3_2",
    FLOW_DIR / "flow.py",
)
...
class InputAndContractTests(unittest.TestCase):
    ...
    def test_manifest_and_selectors_define_frozen_contract(self):
        ...
        self.assertIn("查看签章", selectors["view_sign"])
        self.assertNotIn(".el-tag", selectors["reply_status"])
        self.assertIn("/order/list", (FLOW_DIR / "flow.py").read_text(encoding="utf-8"))
...
class FakePage:
    def __init__(self, locators, *, evaluate_result=""):
        self._locators = locators
        self.evaluate_result = evaluate_result

    def locator(self, selector):
        return self._locators.get(selector, FakeLocator(count=0, visible=False))

    def get_by_text(self, text, exact=False):  # noqa: ANN001
        return self._locators.get(f"text={text}", FakeLocator(count=0, visible=False))

    async def evaluate(self, script, arg=None):
        return self.evaluate_result


class VerifySignedOfficialTests(unittest.IsolatedAsyncioTestCase):
    def _adapter(self, locators, *, evaluate_result="", list_status=""):
        ctx = make_ctx([])
        ctx.selectors = SELECTORS
        ctx.page = FakePage(locators, evaluate_result=evaluate_result)
        adapter = flow_module.SupplierPortalAttachmentAdapter(ctx)
        adapter._list_reply_status = list_status
        return adapter

    async def test_list_column_signed_allows_download_gate(self):
        adapter = self._adapter(
            {SELECTORS["view_sign"]: FakeLocator(visible=True, count=1)},
            list_status="已回签",
        )
        await adapter.verify_signed()
        self.assertEqual(await adapter.reply_status(), "已回签")

    async def test_search_filter_uses_signed_status(self):
        source = (FLOW_DIR / "flow.py").read_text(encoding="utf-8")
        self.assertIn('select_form_option("回复状态", SIGNED_REPLY_STATUS)', source)
        self.assertNotIn("_read_list_reply_status", source)

    async def test_missing_list_status_is_unknown(self):
        adapter = self._adapter({})
        with self.assertRaises(RpaBusinessError) as captured:
            await adapter.verify_signed()
        self.assertEqual(captured.exception.code, "ORDER_NOT_SIGNED")
        self.assertIn("未知", str(captured.exception))
