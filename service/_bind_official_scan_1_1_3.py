"""Switch official-portal scan Binding to 1.1.3 and write searches.

Does not change demo 1.0.x. Preserves browserSession. Does not print passwords.
"""
...
PUBLISH = Path(
    r"d:\work_space260811\autotask-workspace\rpa-flows\rpa_flow_srm_scan_pending_orders\_publish_1.1.3.json"
)
TEMPLATE_CODE = "srm_scan_pending_orders"
...
async def main() -> None:
    ...
    if version != "1.1.3":
        raise SystemExit(f"expected 1.1.3, got {version}")
    ...
    async with async_session_factory() as db:
        ...
        if binding is None:
            raise SystemExit("official scan binding missing")
        config = loads_json(binding.config, {})
        if not isinstance(config, dict):
            config = {}
        config["portalUrl"] = official.portal_url
        config["searches"] = DRILL_SEARCHES
        ...
        print("searches", json.dumps(config.get("searches"), ensure_ascii=False))
        await db.commit()
        ...
        for name, url, bound_version in rows:
            print("scan_binding", name, url, bound_version)
            if DEMO_HOST in (url or "") and not str(bound_version).startswith("1.0"):
                raise SystemExit(f"demo scan binding was changed to {bound_version}")
        print("demo scan bindings unchanged")
