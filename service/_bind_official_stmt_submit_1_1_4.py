"""Bind official statement submit review 1.1.4 with dryRun=true. Does not change demo 1.0.7."""
...
async def main() -> None:
    ...
    async with async_session_factory() as db:
        ...
        if binding is None:
            ...
            db.add(binding)
            print("binding_insert", binding.id, version, "dryRun=true")
        else:
            ...
            print("binding_update", binding.id, version, "dryRun=true")
        ...
        for name, url, bound_version, raw_config in rows:
            ...
            if is_demo_portal(portal) and not str(bound_version).startswith("1.0"):
                raise SystemExit(f"demo submit binding was changed to {bound_version}")
        print("demo submit bindings unchanged")
