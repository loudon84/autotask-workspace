"""Switch srm_prepare_erp_order Binding from 1.2.8 to published Flow 1.2.9."""
...
PUBLISH_JSON = Path(
    ...
    r"\rpa_flow_supplier_portal_prepare_erp_order\_publish_1.2.9.json"
)
...
async def main() -> None:
    ...
    async with async_session_factory() as db:
        ...
        if not rows:
            raise SystemExit("no srm_prepare_erp_order bindings found")
        for binding, template in rows:
            print(
                "before",
                binding.id,
                "portal", binding.portal_account_id,
                "version", binding.rpa_flow_version,
