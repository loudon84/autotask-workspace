"""【墓碑】仅为历史 Binding JSON / scheduler_job_service 解析保留。

Timer 路径（开火、next_run_at、保存校验）已改走 APScheduler + unix_cron 垫片，
新代码禁止 import 本模块；本模块也不再新增功能，随历史路径一起退役。
...
按中国时区墙上时钟推算（调用方传入 now_china()），不跟 4520 机器时区。
"""
