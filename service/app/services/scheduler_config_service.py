...
回签轮询 / 定时扫单 / 京东方匹配交货计划统一为 cron 触发模型：开关 + 5 段 cron
（本地时间）。`*/30 * * * *` = 每半小时，`0 8 * * *` = 每天 8 点，
`0 7 * * *` = 每天 7 点。经 /settings/schedulers API 修改后由调度器热加载，无需重启。
后继任务处理器（SUCCESSOR_JOB_*）不在此列，仍由 .env 控制。
京东方匹配是租户级 HTTP，不写入 Binding 的 scheduler_jobs（避免按门户重复触发）。
"""
...
KEY_BOE_PACK_ENABLED = "scheduler.boePack.enabled"
KEY_BOE_PACK_CRON = "scheduler.boePack.cron"
...
class SchedulerConfig:
    ...
    def parsed_boe_pack(self) -> CronSchedule:
        return CronSchedule.parse(self.boe_pack_cron)
...
def _default_config() -> SchedulerConfig:
    ...
    boe_cron = (
        f"{settings.BOE_PACK_MATCH_JOB_MINUTE} {settings.BOE_PACK_MATCH_JOB_HOUR} * * *"
    )
    return SchedulerConfig(
        ...
        scan_cron=scan_cron,
        boe_pack_enabled=settings.BOE_PACK_MATCH_JOB_ENABLED,
        boe_pack_cron=boe_cron,
    )
...
async def get_scheduler_config(db: AsyncSession) -> SchedulerConfig:
    ...
    keys = [
        KEY_SIGN_POLL_ENABLED,
        KEY_SIGN_POLL_CRON,
        KEY_SCAN_ENABLED,
        KEY_SCAN_CRON,
        KEY_BOE_PACK_ENABLED,
        KEY_BOE_PACK_CRON,
    ]
    rows = (
        await db.execute(
    ...
    return SchedulerConfig(
        ...
        boe_pack_enabled=_bool(KEY_BOE_PACK_ENABLED, defaults.boe_pack_enabled),
        boe_pack_cron=_cron(KEY_BOE_PACK_CRON, defaults.boe_pack_cron),
    )
...
async def update_scheduler_config(
    ...
    CronSchedule.parse(config.boe_pack_cron)
    values = {
        ...
        KEY_BOE_PACK_ENABLED: config.boe_pack_enabled,
        KEY_BOE_PACK_CRON: config.boe_pack_cron,
    }
    existing = {
        row.key: row
        for row in (
